...
export class MedicationDispenseComponent implements OnInit {
    ...
    displayedColumns = ['medication', 'medicationlink', 'dose', 'quantity', 'route','routelink','instructions', 'status', 'prepared', 'handedover',
      'practitioner', 'organisation', 'detail', 'visit', 'resource'];
    ...
    onClick(medicationDispense: fhir.MedicationDispense) {
        ...
        if (this.bundleService.getBundle() !== undefined) {
            ...
            const reference = medicationDispense.medicationReference.reference;
    ...
    showOrganisation(medicationDispense: fhir.MedicationDispense) {
        this.organisations = [];
        for (const performer of medicationDispense.performer) {
    ...
    more(medicationDispense: fhir.MedicationDispense) {
        ...
        const resourceDialog: MatDialogRef<MedicationDispenseDetailComponent> =
          this.dialog.open(MedicationDispenseDetailComponent, dialogConfig);
