...
public class ScheduleDao implements ScheduleRepository {
    ...
    public Schedule create(FhirContext ctx, Schedule schedule, IdType theId, String theConditional) throws OperationOutcomeException  {
        ...
        if (schedule.hasComment()) {
            scheduleEntity.setComment(schedule.getComment());
        }

        log.debug("Schedule.saveCategory");
        if (schedule.hasServiceCategory()) {
            ConceptEntity code = conceptDao.findCode(schedule.getServiceCategory().getCoding().get(0));
            if (code != null) { scheduleEntity.setCategory(code); }
            else {
                log.info("Category: Missing System/Code = "+ schedule.getServiceCategory().getCoding().get(0).getSystem() +" code = "+schedule.getServiceCategory().getCoding().get(0).getCode());

                throw new IllegalArgumentException("Missing System/Code = "+ schedule.getServiceCategory().getCoding().get(0).getSystem()
                        +" code = "+schedule.getServiceCategory().getCoding().get(0).getCode());
            }
        }
